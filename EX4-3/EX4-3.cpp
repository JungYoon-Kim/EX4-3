...
linkedList_h* createLinkedList_h(void) {
	linkedList_h* CL;  CL = (linkedList_h*)malloc(sizeof(linkedList_h));
	CL->head = NULL;
	return CL;
}
...
int main() {
	...
	p = searchNode(CL, (char*)"월"); insertMiddleNode(CL, p, (char*)"수");
	printList(CL); getchar();

	printf("(4) 원형 연결 리스트의 [수] 노드 뒤에 [금] 노드 삽입하기! \n");
	p = searchNode(CL, (char*)"수"); insertMiddleNode(CL, p, (char*)"금");
	printList(CL); getchar();

	printf("(5) 원형 연결 리스트에서 [수] 노드 삭제하기! \n");
	printList(CL); getchar();
	p = searchNode(CL, (char*)"수"); deleteNode(CL, p);
	printList(CL); getchar();
